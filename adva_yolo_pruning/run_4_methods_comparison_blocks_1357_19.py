...
Prunes Conv 0 from blocks: 1, 3, 5, 7, 19 (5 layers)
...
def select_layers_blocks_1_3_5_7_19(detection_model):
    """Select Conv 0 from blocks 1, 3, 5, 7, 19 (5 layers)."""
    target_blocks = [1, 3, 5, 7, 19]
    layers = []
...
def main():
    ...
    # Select target layers (blocks 1, 3, 5, 7, 19)
    targets = select_layers_blocks_1_3_5_7_19(detection_models['method1_activation_max_weight'])
    if len(targets) == 0:
        print("❌ No eligible layers found.")
        return False

    print("\n" + "="*70)
    print("4-METHOD PRUNING COMPARISON EXPERIMENT (BLOCKS 1, 3, 5, 7, 19)")
    print("="*70)
