...
Prunes Conv 0 from blocks: 1, 3, 5, 7, 9 (5 layers)
...
def select_layers_blocks_1_3_5_7_9(detection_model):
    """Select Conv 0 from blocks 1, 3, 5, 7, 9 (5 layers)."""
    target_blocks = [1, 3, 5, 7, 9]
    layers = []
    ...
    try:
        ...
        for param in torch_model.parameters():
            if param.requires_grad:
                ...
                zero_params += (param_flat == 0).sum().item()
...
def main():
    ...
    # Select target layers (blocks 1, 3, 5, 7, 9)
    targets = select_layers_blocks_1_3_5_7_9(detection_models['method1_activation_max_weight'])
    if len(targets) == 0:
        print("❌ No eligible layers found.")
        return False

    # Validate that only expected blocks are selected
    expected_blocks = {1, 3, 5, 7, 9}
    ...
    if unexpected_blocks:
        ...
        print(f"   This suggests Block 9 might not have Conv layers or the selection function has a bug")
    ...
    print("\n" + "="*70)
    print("4-METHOD PRUNING COMPARISON EXPERIMENT (BLOCKS 1, 3, 5, 7, 9)")
    print("="*70)
