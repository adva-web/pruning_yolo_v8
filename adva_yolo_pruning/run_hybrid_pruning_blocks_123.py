#!/usr/bin/env python3
"""
Hybrid Pruning Experiment: Blocks 1, 2, 3
- Block 1: Activation pruning (regular Conv block)
- Block 2: Gamma pruning (C2f block)
- Block 3: Activation pruning (regular Conv block)
...
def main():
    ...
    print("=" * 80)
    print("HYBRID PRUNING EXPERIMENT: BLOCKS 1, 2, 3")
    print("=" * 80)
    print("Strategy:")
    print("  - Block 1: Activation pruning (regular Conv block)")
    print("  - Block 2: Gamma pruning (C2f block)")
    print("  - Block 3: Activation pruning (regular Conv block)")
    print("=" * 80)
    ...
    for block_idx in target_blocks:
        if block_idx < len(detection_model):
            ...
            print(f"   Block {block_idx}: {block_type}")
            
            if block_idx == 2 and not is_c2f:
                print(f"   ⚠️  WARNING: Block 2 is not C2f, but we'll use gamma pruning anyway")
            if block_idx in [1, 3] and is_c2f:
    ...
    # Save pruned model
    output_path = "runs/detect/hybrid_pruning_blocks_123.pt"
