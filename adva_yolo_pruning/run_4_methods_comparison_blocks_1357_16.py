...
Prunes Conv 0 from blocks: 1, 3, 5, 7, 16 (5 layers)
...
def select_layers_blocks_1_3_5_7_16(detection_model):
    """Select Conv 0 from blocks 1, 3, 5, 7, 16 (5 layers)."""
    target_blocks = [1, 3, 5, 7, 16]
    layers = []
...
def main():
    ...
    # Select target layers (blocks 1, 3, 5, 7, 16)
    targets = select_layers_blocks_1_3_5_7_16(detection_models['method1_activation_max_weight'])
    if len(targets) == 0:
        print("❌ No eligible layers found.")
        return False

    print("\n" + "="*70)
    print("4-METHOD PRUNING COMPARISON EXPERIMENT (BLOCKS 1, 3, 5, 7, 16)")
    print("="*70)
