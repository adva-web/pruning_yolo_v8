#!/usr/bin/env python3
"""
Robust Channel Fix for Multi-Layer Pruning
This module provides a more comprehensive solution for channel mismatch errors.
"""
...
def apply_robust_activation_pruning(model_path, train_data, valid_data, classes, layers_to_prune=3, data_yaml="data/VOC_adva.yaml"):
    """
    Apply activation-based pruning with a robust channel mismatch fix.
    This approach prunes layers one at a time and updates the model architecture after each pruning step.
    """
    if layers_to_prune < 1 or layers_to_prune > 12:
        raise ValueError("layers_to_prune must be between 1 and 12")
    
    print(f"\n===== Robust Activation-based Pruning with Channel Fix =====")
    print(f"🔧 Layers to prune: {layers_to_prune}")
    print(f"🔧 This method prunes one layer at a time to avoid channel mismatches")
    ...
    # Apply activation-based pruning to selected layers ONE AT A TIME
    ...
    for idx, conv_info in enumerate(selected_convs):
        ...
        try:
            ...
            # CRITICAL: After pruning this layer, we need to update the model architecture
            # to ensure subsequent layers have the correct input channel expectations
            print(f"  🔧 Updating model architecture after pruning layer {idx + 1}...")
            update_success = update_model_architecture_after_layer_pruning(detection_model, conv_info, channels_to_keep)
            
            if update_success:
                print(f"  ✅ Model architecture updated successfully")
            else:
                print(f"  ⚠️  Model architecture update failed, but continuing...")
    ...
    print(f"\n✅ Robust activation-based pruning completed successfully!")
    print(f"📊 Successfully pruned {successfully_pruned_layers}/{len(selected_convs)} layers")
    return model

def update_model_architecture_after_layer_pruning(detection_model, pruned_conv_info, new_output_channels):
    """
    Update the model architecture after pruning a single layer.
    This function ensures that all subsequent layers have the correct input channel expectations.
    """
    try:
        ...
        print(f"    📝 Updating architecture after pruning layer (Block {block_idx}, Conv {conv_in_block_idx})")
        ...
        # Update all subsequent layers that might be affected
        layers_updated = 0
...
if __name__ == "__main__":
    print("🧪 Testing robust channel fix...")
    print("This module provides a more comprehensive solution for channel mismatch errors.")
    print("Use apply_robust_activation_pruning() to test the fix.")
