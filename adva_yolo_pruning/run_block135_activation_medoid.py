#!/usr/bin/env python3
"""
Experiment 1: Activation-based pruning with MEDOID selection
Prunes Conv 0 from blocks 1, 3, 5 using activation algorithm but selects MEDOIDS 
instead of max-weight channels from each cluster.

This allows comparing:
- Medoid selection (geometric center of cluster)
...
from clustering_variants import select_optimal_components_medoid
...
def prune_conv2d_with_activations_medoid(model_path, train_data, valid_data, classes, 
                                         block_idx, conv_in_block_idx, data_yaml):
    """
    Prune Conv2d using activation algorithm with MEDOID selection (not max weight).
    Same as original but uses select_optimal_components_medoid.
    """
    ...
    print("   🔍 Using MEDOID selection (geometric center, not max weight)")
    graph_space = YoloLayerPruner(activations=train_activations).create_layer_space()
    layer_weights = target_conv_layer.weight.data.detach().cpu().numpy()  # Used for compatibility but not for selection
    
    # Target pruning ratio
    num_channels = target_conv_layer.weight.shape[0]
    target_channels = max(num_channels // 2, num_channels // 4)
    
    # Use MEDOID selection (returns medoids directly)
    ...
    print(f"   📊 Activation analysis (MEDOID selection):")
    ...
    print(f"   ✅ Activation-based pruning (MEDOID) applied successfully!")
    return model
...
def main():
    ...
    print("\n" + "="*70)
    print("EXPERIMENT 1: ACTIVATION PRUNING WITH MEDOID SELECTION")
    print("="*70)
    ...
    # Iterate and prune with medoid selection
    for i, t in enumerate(targets):
        print(f"\n{'='*70}\nITERATION {i+1}/{len(targets)}\n{'='*70}")
        print(f"   🔧 Pruning {t['name']} using MEDOID selection")
        
        # Save model state and prune independently
        temp_path = f"temp_medoid_iter_{i+1}.pt"
        model.save(temp_path)
        
        try:
            updated_model = prune_conv2d_with_activations_medoid(
    ...
    # Summary
    print("\n" + "="*70)
    print("FINAL SUMMARY (ACTIVATION + MEDOID SELECTION)")
    print("="*70)
